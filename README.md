...
- **직접 푸시 거부**: 드래프트를 추가한 커밋이 PR 머지가 아니면(직접 푸시)
  발행하지 않고 경고만 남긴다 — 머지(=오너 승인)를 우회한 발행 차단.
- **긴급 정지**: 레포 루트에 `.publish-paused` 파일이 있으면(노드 로컬 생성
  또는 main에 커밋) 모든 자동 발행이 멈춘다. 파일 삭제로 재개.
